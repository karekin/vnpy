...
- 这里列的是当前已经进入运行链路的有效字段
- 未生效的历史预留字段已经从 `runtime_config` 和前后端任务接口中删除
- 独立回测任务面板当前也不再接收手续费参数，滑点只保留在前端实验面板展示，不进入后端计算
...
2. 若后续要引入真实资金曲线与交易成本，再重新设计并引入独立的手续费、滑点字段，而不是恢复旧接口参数
