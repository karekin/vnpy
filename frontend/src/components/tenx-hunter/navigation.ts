...
export type TenxNavItem = {
  name: string;
  path: string;
  group: "overview" | "pipeline" | "context";
  step?: string;
  stageLabel?: string;
};
...
export function tenxNavItems(market: string): TenxNavItem[] {
  ...
  if (market.toUpperCase() === "US") {
    return [
      { name: "Workspace", path: base, group: "overview", stageLabel: "总览" },
  ...
  return [
    ...
    { name: "Watchlist", path: `${base}/watchlist`, group: "pipeline", step: "02", stageLabel: "观察" },
    { name: "Alerts", path: `${base}/alerts`, group: "pipeline", step: "03", stageLabel: "提醒" },
    { name: "Themes", path: `${base}/themes`, group: "context", stageLabel: "主题" },
  ];
}
