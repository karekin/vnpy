...
import { tenxNavItems } from "@/components/tenx-hunter/navigation";
import clsx from "clsx";
import Link from "next/link";
import { usePathname } from "next/navigation";

export default function TenxNavigation() {
  const pathname = usePathname();

  return (
    <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="flex flex-wrap gap-2">
        {tenxNavItems.map((item) => {
          const active =
            item.path === "/tenx-hunter"
              ? pathname === item.path || pathname.startsWith("/tenx-hunter/research/")
